...
const Demo2 = defineComponent({
    setup() {
        ...
        return () => (<Card title="手动触发" class="mt-2">
                <Typography>
                    <Typography.Paragraph>
                        如果设置了
...
const Demo4 = defineComponent({
    setup() {
        ...
        return () => (<Card title="刷新（重复上一次请求）" class="mt-2">
                <Typography>
                    <Typography.Paragraph>
                        <Typography.Text type="danger">useRequest </Typography.Text>
                        提供了
        ...
        return () => (<Card title="取消响应" class="mt-2">
                <Typography>
                    <Typography.Paragraph>
                        ...
                        <Typography.Text type="danger"> cancel </Typography.Text>函数，用于忽略当前
                        promise 返回的数据和错误
                    </Typography.Paragraph>
                </Typography>
...
const Demo6 = defineComponent({
    setup() {
        const search = ref('');
        const { data: username, loading, run, params } = useRequest(imitateApi, {
            defaultParams: ['lutz']
        });
        const onChange = () => {
            run(search.value);
        };
        return () => (<Card title="管理参数" class="mt-2">
                <Typography>
                    <Typography.Paragraph>
                    ...
                    <Typography.Paragraph>
                        如果我们设置了
                        <Typography.Text type="danger"> options.manual = false </Typography.Text>
                        ，则首次调用
                        <Typography.Text type="danger"> service </Typography.Text>
                        的参数可以通过
                        <Typography.Text type="danger"> options.defaultParams </Typography.Text>
                        来设置。
                    </Typography.Paragraph>
                </Typography>
